 ...
 * @since 1.0.2
 */
function wpforms_register_widgets() { // phpcs:ignore Universal.Files.SeparateFunctionsFromOO.Mixed, WPForms.PHP.HooksMethod.InvalidPlaceForAddingHooks

	// The widget should be registered for WP version less than 5.8 when the Classic Widgets plugin is not active.
	if (
		version_compare( get_bloginfo( 'version' ), '5.8', '>=' ) &&
		! in_array( 'classic-widgets/classic-widgets.php', (array) get_option( 'active_plugins', [] ), true )
	) {

		// BC: bail out early if the widget is not displayed on the front end.
		if ( ! is_active_widget( false, false, 'wpforms-widget' ) ) {
			// Widget won't be registered.
			return;
		}

		// If the widget is active, we show it on the sidebar,
		// but make sure new legacy widgets cannot be added.
		add_filter(
