...
class WPForms_Field_Select extends WPForms_Field {
	...
	public function format( $field_id, $field_submit, $form_data ) {
		...
		if ( $dynamic === 'post_type' && ! empty( $field['dynamic_post_type'] ) ) {
		...
		} elseif ( ! empty( $field['show_values'] ) && (int) $field['show_values'] === 1 ) {
			...
			foreach ( $field_submit as $item ) {
				foreach ( $field['choices'] as $key => $choice ) {
					if ( in_array( $item, [ $choice['value'], $choice['label'] ], true ) || ( empty( $choice['value'] ) && $key === (int) preg_replace( '/\D/', '', $item ) ) ) {
						$choice_value = ! empty( $item ) ? $item : $choice['label'];
						$value[]      = sanitize_text_field( $choice_value );
