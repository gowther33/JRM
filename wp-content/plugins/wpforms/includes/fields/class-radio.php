		...
		if ( $dynamic === 'post_type' && ! empty( $field['dynamic_post_type'] ) ) {
			...
			if ( ! empty( $field['show_values'] ) ) {
				foreach ( $field['choices'] as $key => $choice ) {
					if ( ( ! empty( $field_submit ) && $choice['value'] === $field_submit ) || ( empty( $choice['value'] ) && $key === (int) preg_replace( '/\D/', '', $field_submit ) ) ) {
						$choice_value  = ! empty( $field_submit ) ? $field_submit : $choice['label'];
						$data['value'] = sanitize_text_field( $choice_value );
						$choice_key    = $key;
				...
				foreach ( $field['choices'] as $key => $choice ) {
					/* translators: %s - choice number. */
					if ( $field_submit === $choice['label'] || $key === (int) preg_replace( '/\D/', '', $value_raw ) ) {
						$choice_key = $key;
